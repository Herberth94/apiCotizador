...
export const EditAM = () => {
    ...
    async function Send (dataPar,dataCat,amId,newdata){
        ...
        try {
            ...
            if(dataPar !== ''){
                await axios.post(url2 + `/api/cotizador/am/EditAMPar/${amId}`,dataActualizacion);
                alert('Datos AM de una Partida editados exitosamente');
            }else{
                await axios.post(url2 + `/api/cotizador/am/EditAMCats/${amId}`,dataActualizacion1);
                alert('Datos AM de una Categoria editados exitosamente');
            }
            } catch (error) {
            console.log(error);

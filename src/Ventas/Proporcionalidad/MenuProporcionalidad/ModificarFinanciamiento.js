...
function ModificarFinanciamiento(prop) {
  ...
  async function llamadiListaFinanciamiento() {
    ...
    try {
      const respuesta = await axios.get(
        url2 + `/api/cotizador/proporcionalidad/view/${prop.propIdProyecto}`
      );
  ...
  async function SendProporcionalidadModificada(oldData) {
    ...
    if (estatusProy2 === "Aceptado") {
      alert(
        "No se puede modificar este Proyecto, se encuentra en Estatus: Aceptado"
      );
    } else {
      try {
        //console.log('Data actualización:',data);
        await axios.put(
          url2 + `/api/cotizador/proyecto/updateFM/${idAsignado}`,
          dataFM
        );
        const respuestaUpdate = await axios.put(
          url2 +
            `/api/cotizador/proporcionalidad/update/${prop.propIdProyecto}`,
          data
        );
        ...
        <Tbody>
          {Object.keys(listaFinanciamiento).map((key) => (
            <Tr key={listaFinanciamiento[key].pd_id}>
              {/*================= ID ==================*/}
              <Td>{listaFinanciamiento[key].pd_id}</Td>

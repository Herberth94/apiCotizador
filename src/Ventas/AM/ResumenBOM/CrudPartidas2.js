...
export const CrudPartidas2 = (props) => {
  ...
  async function SendDeletePartida(id) {
    ...
    if (confirmacion) {
      ...
      for (let c = 0; c < cSP; c++) {
        try {
          await axios.delete(
            url2 + `/api/cotizador/precio/delete/${listaSP[c].sp_id_precio}`
          );
          setShow(!show);
        } catch (error) {}
      }
  ...
  async function getDatosSP(partida_id) {
    try {
      const resPSP = await axios.get(
        url2 + `/api/cotizador/sp/viewPSP/${partida_id}`
      );
        ...
        <Tbody>
          {Object.keys(props.partidas).map((key) => (
            ...
            <Tr key={key}>
              ...
              <Td>{props.partidas[key].partida_nombre}</Td>
              <Td>{props.partidas[key].partida_descripcion}</Td>

  ...
  async function getDatosPrecios(sp_id) {
    try {
      const resPrecSP = await axios.get(
        url2 + `/api/cotizador/precio/viewSPP/${sp_id}`
      );
        ...
        <Tbody>
          {Object.keys(props.sp).map((key) => (
            <Tr key={key}>
              ...
              <Td>{props.sp[key].spnp_np}</Td>
              <Td>{props.sp[key].spd_des}</Td>
              <Td>{props.sp[key].sp_meses}</Td>
              <Td>{props.sp[key].sp_semanas}</Td>
              <Td>{nombreProv[key]}</Td>
              <Td>{nombreMarca[key]}</Td>
              <Td>
                {" "}
                <select
              ...
              <Td>{props.sp[key].sp_comentarios}</Td>
              {/*    <td>
                                    <button 

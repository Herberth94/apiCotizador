...
export const CrudProyectos = (props) => {
  const { getTotalPar } = Partida_catalogo();

  async function consultarTotalesP() {
    getTotalPar("");
    try {
      const resTotPar = await axios.get(
        url2 + `/api/cotizador/am/viewTotalesPartidas/${pId}`
      );
  ...
  const onChangeTextCliente = (nombreCliente, key) => {
    ...
    console.log(
      "Cliente especifico:",
      suggestionsClientes[0][0].nombre_cliente
    );
  };
  ...
  function getIdP(proyecto) {
    ...
    setProyecto(
      `Proyecto: FO-ING-01 BOM - ${proyecto.proyecto_clave} - ${proyecto.proyecto_descripcion}  -  ${proyecto.nombre_cliente}`
    );
  ...
  async function getDatosPartida(id) {
    try {
      const resPP = await axios.get(
        url2 + `/api/cotizador/partida/viewPP/${id}`
      );
  ...
  async function getDatosCats(proyecto_id) {
    try {
      const resCtsP = await axios.get(
        url2 + `/api/cotizador/catd/view/${proyecto_id}`
      );
        ...
        <Tbody>
          {Object.keys(props.suggestionsP).map((key) => (
            <Tr key={props.suggestionsP[key].proyecto_id}>
              ...
              <Td className={props.suggestionsP[key].proyecto_estatus}>
                {props.suggestionsP[key].proyecto_estatus}
              </Td>
              <Td>
                <input

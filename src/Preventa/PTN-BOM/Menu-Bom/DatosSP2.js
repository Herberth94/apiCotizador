...
function DatosSp2(props) {
    ...
    if (l === 1 && firtsCopy === false) {
      ...
      setRows([
        ...rows,
        {
          id: rows.length + 1,
          n_parte: sp.spnp_np,
          descripcion: sp.spd_des,
          meses: sp.sp_meses,
          semanas: sp.sp_semanas,
          cantidad: sp.sp_cantidad,
          precio_lista: sp.precio_lista,
          precio_unitario: sp.precio_unitario,
          precio_descuento: sp.precio_descuento,
          total: sp.precio_total,
          moneda: sp.precio_id_moneda,
          categoria: sp.sp_id_categoria,
          proveedor: sp.proveedor_nombre,
          marca: sp.marca_nombre,
          comentarios: sp.sp_comentarios,
        },
      ]);
    }
    setFirtsCopy(true);
  ...
  const lista = async (clave) => {
    try {
      const respuesta = await axios.get(
        url2 + `/api/cotizador/catd/view/modal/${clave}`
      );
  ...
  const guardarListaSP = async () => {
    ...
    if (pEstatus1 === "En revision") {
      ...
      try {
        await axios.put(
          url2 + `/api/cotizador/proyecto/updateFM/${props.clave}`,
          dataFM
        );

        if (parId !== "" && parId !== undefined) {
          const respuesta = await axios.post(
            url2 + `/api/cotizador/sp/insercionMultiple/${parId}`,
            rows
          );
          alert(respuesta.data.msg);
        } else {
          const respuesta = await axios.post(
            url2 + `/api/cotizador/sp/insercionMultiple/${idPartidaInsertada}`,
            rows
          );
          alert(respuesta.data.msg);
        }

      ...
      for (let cont = 0; cont < i; cont++) {
        if (
          ...
          parseInt(validatorid) ===
            parseInt(resUsers.data.reSql[cont].id_usuario)
        ) {
          const clave = await axios.post(
            url2 + `/api/cotizador/proyecto/agregar/${validatorid}`,
            data
          );
              ...
              {suggestions &&
                suggestions.map((suggestion, i) => (
                  <div
                    ...
                    onClick={() => onSuggestHandler(suggestion.nombre_cliente)}
                  >

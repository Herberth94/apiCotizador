...
function DatosSP({ clave }) {
    ...
    async function listaProvs() {
      try {
        const respuesta = await axios.get(
          url + "/api/cotizador/proveedor/view"
        );
  ...
  useEffect(() => {
    ...
    async function listaMarcas() {
      try {
        const respuesta = await axios.get(
          url2 + `/api/cotizador/provmarcas/view/${proveedorId.proveedor_id}`
        );
  ...
  function copyDataSP(key) {
    setDatosSP({
      ...datosSP,
      sp_no_parte: suggestions[key].spnp_np,
    ...
    setDatosCategoria({
      ...datosCategoria,
      categoria_id: suggestions[key].sp_id_categoria,
    });

    setDatos({
      ...datos,
      precio_lista: suggestions[key].precio_lista,
  ...
  async function SendSP() {
    ...
    if (pEstatus1 === "En revision") {
      alert(
        "No se puede continuar el Proyecto porque se encuentra En revision"
      );
      ...
      try {
        ...
        if (pId2 !== pId && pId2 !== "") {
          await axios.put(
            url2 + `/api/cotizador/proyecto/updateFM/${pId2}`,
            dataFM
          );
        } else {
          await axios.put(
        ...
        const resPrecio = await axios.post(
          url + "/api/cotizador/precio/agregar",
          dataPrecio
        );
        ...
        const resGetPartida = await axios.get(
          url2 + `/api/cotizador/partida/viewPU/${validatorid}`
        );
        ...
        if (parId !== partidaId.partida_id && parId !== "") {
          if (spnp.spnp_id !== "") {
            ...
            const resSpnp = await axios.post(
              url + "/api/cotizador/sp/agregarSpnp",
              dataSpnp
            );
          ...
          if (spd.spd_id !== "") {
            ...
            const resSpd = await axios.post(
              url + "/api/cotizador/sp/agregarSpd",
              dataSpd
            );
          ...
          const respuesta = await axios.post(
            url2 +
              `/api/cotizador/sp/agregar/${parId}/${proveedorId.proveedor_id}/${marcaId.marca_id}`,
            dataSP
          );
          ...
          const respuesta = await axios.post(
            url2 +
              `/api/cotizador/sp/agregar/${partidaId.partida_id}/${proveedorId.proveedor_id}/${marcaId.marca_id}`,
            dataSP
          );
          const respuestaBack = respuesta.data.msg;
          //console.log(respuestaBack)
        ...
        alert(
          "Registro de Servicio/producto invalido, revisa que hayas seleccionado correctamente el proveedor y la marca, y el tipo de moneda y categoría"
        );
        //console.log(error);
      }
    }
          ...
          <tbody>
            <tr className="">
              ...
              <td>
                ...
                />
                {suggestionsProv &&
                  suggestionsProv.map((suggestionProv, i) => (
                    <div
                      key={i}
                      className="selectCliente"
                      onClick={() =>
                        onSuggestHandlerProv(suggestionProv.proveedor_nombre)
                      }
                    >
              ...
              <td>
                ...
                />
                {suggestionsMarca &&
                  suggestionsMarca.map((suggestionMarca, i) => (
                    <div
                      key={i}
                      className="selectCliente"
                      onClick={() =>
                        onSuggestHandlerMarca(suggestionMarca.marca_nombre)
                      }
                    >
              ...
              <td>
                ...
                <select
                  ...
                  value={datosCategoria.categoria_id}
                >
                  <option value={0}></option>

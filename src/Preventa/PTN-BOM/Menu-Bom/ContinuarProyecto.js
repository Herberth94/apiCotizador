  ...
  const getProyectos = async () => {
    try {
      if (validatorrol === "direccion") {
        ...
        if (
          URLactual === "http://localhost:3000/mis-proyectos" ||
          URLactual === "http://10.200.10.9:3000/mis-proyectos"
        ) {
          const resProy = await axios.get(
          ...
          URLactual === "http://localhost:3000/proyectos-compartidos" ||
          URLactual === "http://10.200.10.9:3000/proyectos-compartidos"
        ) {
          const resProy = await axios.get(
  ...
  async function getDatosPartida(proyecto_id) {
    try {
      const resPP = await axios.get(
        url2 + `/api/cotizador/partida/viewPP/${proyecto_id}`
      );
          ...
          <tbody>
            <tr>
              <td>
                ...
                >
                  {show2 ? "Agregar Nueva Partidas" : "Ocultar"}{" "}
                </button>
              </td>
              <td>
                <button
                ...
                >
                  {show3 ? "Continuar Partida" : "Ocultar"}{" "}
                </button>
              </td>
            </tr>
          </tbody>

  ...
  async function Send() {
    ...
    if (idUsuariosState !== undefined) {
      if (idProyectoState !== undefined) {
        if (passwordState !== "") {
          try {
            const respuesta = await axios.post(
              url + "/api/cotizador/colaboradores/insert",
              mandarInformacion
            );
        ...
        alert(
          "¡ERROR! Ingresa la clave del proyecto al que quieres añadir un colaborador"
        );
      }
    } else {
    ...
    <div className="contenido-main-registro">
      <div className="scene flex">
        <section className="card-body">
          ...
          />
          {suggestionsUsuarios &&
            suggestionsUsuarios.map((suggestionU, i) => (
              <div
                key={i}
                className="selectCliente"
                onClick={() =>
                  onSuggestHandlerEmail(
                    suggestionU.email,
                    suggestionU.id_usuario
                  )
                }
              >
                {suggestionU.email}
          ...
          />
          {suggestionsProyecto &&
            suggestionsProyecto.map((suggestionP, i) => (
              <div
                key={i}
                className="selectCliente"
                onClick={() =>
                  onSuggestHandlerProyecto(
                    suggestionP.proyecto_clave,
                    suggestionP.proyecto_id
                  )
                }
              >
                {suggestionP.proyecto_clave}

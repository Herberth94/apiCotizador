    ...
    if (proveedorId.proveedor_id !== "" && datosMarca.marca_nombre !== "") {
      try {
        ...
        for (let c = 0; c < i1.length; c++) {
          if (
            datosMarca.marca_nombre.toUpperCase() ===
            resMarcas.data.data[c].marca_nombre.toUpperCase()
          ) {
        ...
        if (marcaId.marca_id !== "") {
          const respuesta = await axios.post(
            url2 +
              `/api/cotizador/marca/agregarPM/${proveedorId.proveedor_id}/${marcaId.marca_id}`
          );
          ...
          const respuesta = await axios.post(
            url2 + `/api/cotizador/marca/agregar/${proveedorId.proveedor_id}`,
            data
          );
          const respuestaBack = respuesta.data.msg;
          //console.log(respuestaBack)

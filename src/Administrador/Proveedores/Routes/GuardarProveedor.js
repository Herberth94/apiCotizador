  ...
  async function SendProveedor() {
    ...
    if (data.proveedor_nombre.toUpperCase() !== "") {
      ...
      try {
        ...
        for (let c = 0; c < k; c++) {
          if (
            datosProv.proveedor_nombre.toUpperCase() ===
            resProvs.data.data[c].proveedor_nombre.toUpperCase()
          ) {
        ...
        if (insertar) {
          const respuesta = await axios.post(
            url + "/api/cotizador/proveedor/agregar",
            data
          );
      ...
      alert(
        "Error en el registro, verifica que los datos ingresados sean correctos"
      );
    }
  }
